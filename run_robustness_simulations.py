...
def solve_mf(gamma, p1, p2, tau, lam=1.0):
...
def mc_run(nbrs, gamma, p1, p2, tau, burn_per_node, measure_per_node, realizations, rng):
    n = len(nbrs)
    ...
    for _ in range(realizations):
        ...
        vals.append(run_phase(cap, measure_steps, accumulate=True) / (n * measure_steps))
    return float(np.mean(vals))


def simulate_curve(spec, gammas, params, reps=3, k_per_rep=8, burn_per_node=BURN_PER_NODE, measure_per_node=MEASURE_PER_NODE):
    rows = []
    means = []
    sems = []
    for gamma in gammas:
...
def main():
    ...
    axes[0].legend(fontsize=10.8, framealpha=0.92, loc="lower left")
    axes[1].legend(fontsize=10.8, framealpha=0.92, loc="upper right")
    axes[2].legend(fontsize=10.8, framealpha=0.92, loc="lower left")
