...
def pair_equations(x, k, gamma, p1=P1, p2=P2, tau=TAU):
    ...
    for ei, (a, b) in enumerate(pairs):
        ...
        # Direct i-j contests include the independent penalty on the focal
        # endpoint, including no-state-change branches in mod-3 space.
        direct_i = (
            0.5 * (1 - tau) * p[ad, bu]
            + 0.5 * tau * p[a, bu]
            + 0.5 * (1 - tau) * p[au, bd]
        ...
        flux += gamma / k * (direct_i + direct_j)

        other_i = 0.5 * p[ad, b] + 0.5 * (1 - tau) * p[au, b] + 0.5 * tau * pab - pab
        other_j = 0.5 * p[a, bd] + 0.5 * (1 - tau) * p[a, bu] + 0.5 * tau * pab - pab
        flux += gamma * (k - 1) / k * (other_i + other_j)
        eqs[ei] = flux
...
def load_or_compute_data():
    ...
    gammas2 = np.r_[np.linspace(0.02, 0.95, 18), 1.0]
    data["fig2_k"] = k_list
    data["fig2_g"] = gammas2
    for k in k_list:
...
def _make_fig1_v14():
    ...
    ax_cond.text(0.50, 0.57, r"$\pi_0^{\mathrm{mix}} < \pi_0^\ast$", ha="center", fontsize=15.0, color="#14532D")
...
def make_fig1():
    ...
    ax_cond.text(0.50, 0.58, r"$\pi_{0}^{\mathrm{mix}}<\pi_{0}^{\ast}$", ha="center", fontsize=16.0, color="#14532D")
...
def make_fig4(data):
    ...
    for i, p_r in enumerate(pr):
        if np.isclose(p_r, pr.min()) or np.isclose(p_r, pr.max()):
            continue
